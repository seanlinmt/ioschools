<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<ioschoolsWebsite.Models.eca.ECARow>" %>
<tr alt="<%= Model.eca.id %>" class="bg_edit">
    <td>
        <%= Html.DropDownList("schools", Model.schoolList) %>
    </td>
    <td>
        <%= Html.TextBox("name", Model.eca.name, new Dictionary<string, object>(){{"class", "w400px"}}) %>
    </td>
    <td class="ar">
