...
using ioschoolsWebsite.Library.email;
using ioschoolsWebsite.Library.Lucene;
using ioschoolsWebsite.Models.email;
using ioschoolsWebsite.Models.enrolment;
            ...
            string term)
        {
            ...
            if (!string.IsNullOrEmpty(term))
            {
                ...
                else
                {
                    ...
                    results = results.Where(x => ids.Select(y => y.id).Contains(x.user.id.ToString())).AsEnumerable();
                    results = results.Join(ids, x => x.user.id.ToString(), y => y.id, (x, y) => new { x, y.score })
